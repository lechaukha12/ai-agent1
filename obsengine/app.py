...
try:
    ...
    sys.exit(f"Core module import failed: {e}") # Exit if core modules fail
except Exception as e:
    logger.critical(f"Unexpected error during imports: {e}", exc_info=True)
    sys.exit("Unexpected error during imports.") # Exit on other import errors
...
try:
    ...
    sys.exit("Flask app creation failed.") # Exit if Flask fails
...
try:
    ...
    try:
     ...
     # Do not exit here, allow fallback to DB cache for namespaces
...
try:
    ...
    DB_PATH = "/data/fallback_obsengine_data.db" # Fallback path
    logger.warning(f"Using fallback DB path: {DB_PATH}")

# --- Get Initial DB Defaults (for global settings) ---
def get_initial_global_db_defaults():
     """Gets default values ONLY for GLOBAL settings."""
...
try:
    if not db_manager.init_db(DB_PATH, get_initial_global_db_defaults()):
        # Log critical error but don't exit immediately, maybe Flask can still serve some routes
    ...
    # Log critical error but don't exit immediately
...
def periodic_background_tasks_thread():
    """Runs periodic tasks like updating stats and cleaning up inactive agents."""
    try:
...
def collect_data():
    ...
    if not request.is_json: logger.error("Received non-JSON request to /collect"); return jsonify({"error": "Request must be JSON"}), 400
    try: data = request.get_json(); logger.debug(f"Received raw data (keys): {list(data.keys())}")
    except Exception as e: logger.error(f"Error getting or parsing JSON data: {e}", exc_info=True); return jsonify({"error": "Failed to parse JSON data"}), 400

    required_fields = ["pod_key", "k8s_context", "logs", "initial_reasons", "collection_timestamp", "agent_id", "cluster_name"]; missing_fields = [field for field in required_fields if field not in data]
    if missing_fields: error_msg = f"Missing required fields: {', '.join(missing_fields)}"; logger.error(f"Validation failed: {error_msg}"); return jsonify({"error": error_msg}), 400

    pod_key = data.get("pod_key") # Should be namespace/pod_name from agent
    k8s_context = data.get("k8s_context")
    logs = data.get("logs", [])
    initial_reasons = data.get("initial_reasons", [])
    agent_id = data.get("agent_id") # Agent ID (e.g., cluster name or UUID)
    cluster_name = data.get("cluster_name") # Cluster name
    ...
    try: db_manager.update_agent_heartbeat(DB_PATH, agent_id, cluster_name, received_time.isoformat())
    except Exception as heartbeat_err: logger.error(f"Failed to update heartbeat for agent {agent_id}: {heartbeat_err}", exc_info=True)
    ...
    try:
        ...
        db_path_local = obsengine_config.get_db_path() # Get potentially overridden path

        # --- TODO: Load AGENT-SPECIFIC configurations and MERGE ---
        # This is the crucial part missing for agent-specific behavior
        # agent_config = db_manager.load_agent_config(db_path_local, agent_id)
        # merged_ai_config = {**global_ai_config, **agent_config} # Needs refinement for specific keys
        # merged_alert_config = {**global_alert_config, **agent_config} # Needs refinement
        # For now, using global config:
        current_ai_config = global_ai_config
        current_alert_config = global_alert_config
        # ---

        logger.debug(f"[{full_pod_key}] Performing analysis (using {'agent' if 'agent_config' in locals() else 'global'} config)...")
        ...
        # Use global alert levels for incident stats counting for now
        ...
        # --- Check alert conditions (using CURRENT config) ---
        ...
        if severity in alert_levels:
            ...
            if not db_manager.is_pod_in_cooldown(db_path_local, full_pod_key):
                ...
                # Use GLOBAL Telegram settings for sending alerts
...
def get_all_config_api():
    ...
    try:
        ...
        # Add sensitive keys from env vars if they exist
        ...
        # Global namespaces are not really used anymore, return empty list
...
def get_available_namespaces_api():
    ...
    if k8s_client_initialized_obs and k8s_core_v1_obs:
        try: # <--- Start of the try block
            ...
            if hasattr(all_namespaces, 'items') and all_namespaces.items is not None: # <--- Line 397 (approx)
                 for ns in all_namespaces.items:
                     if ns.status.phase == "Active" and ns.metadata.name not in excluded_namespaces:
                         namespaces.append(ns.metadata.name)
            else: # <--- Corresponding else block
            ...
            # Update DB cache in background maybe? For now just return live data.
            # threading.Thread(target=db_manager.update_available_namespaces_in_db, args=(DB_PATH, namespaces), daemon=True).start()
            return jsonify(namespaces), 200 # <--- End of the try block before except
        except ApiException as e: # <--- First except block
            logger.warning(f"K8s API error fetching namespaces: {e.status} {e.reason}. Falling back to DB cache.")
        except Exception as e: # <--- Second except block
...
# --- Global Config Save Endpoints (AI, Telegram) ---
...
def save_global_ai_config_api():
    ...
    if success: obsengine_config._last_config_refresh_time = 0; return jsonify({"message": message}), 200 # Force refresh
...
# --- Agent-Specific Configuration API Endpoints ---
...
def get_agent_config_api(agent_id):
    ...
    try:
        ...
        merged_config = {
            ...
            'monitored_namespaces': [], # Default to empty for agent
        }
...
if __name__ == '__main__':
    ...
    # Important: use_reloader should be False when running with Gunicorn
