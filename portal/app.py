...
def init_db():
    ...
    try:
        with conn:
            ...
            # Define incidents table structure (matching agent's v1.0.6 definition)
            cursor.execute('''
            ...
            ''')
            # Define daily_stats table structure
            cursor.execute('''
...
@login_required
def get_incidents():
    pod_filter = request.args.get('pod', default="", type=str).strip(); severity_filter = request.args.get('severity', default="", type=str).upper().strip(); start_date_str = request.args.get('start_date', default=None, type=str); end_date_str = request.args.get('end_date', default=None, type=str); page = request.args.get('page', 1, type=int); per_page = request.args.get('per_page', 20, type=int); offset = (page - 1) * per_page
    conn = get_db_connection();
    ...
    try:
        ...
        # v1.0.6: Select all columns needed for the details modal
        data_query = f"""
        ...
        # *** FIX: Separate statements in except block ***
...
def manage_monitored_namespaces():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            logging.info(f"Updated monitored_namespaces config in DB: {value_to_save}"); return jsonify({"message": "Configuration saved successfully."}), 200
        except sqlite3.Error as e:
