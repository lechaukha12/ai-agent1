...
k8s_core_v1 = None
k8s_apps_v1 = None
# --- ADDED: Version API client ---
k8s_version_api = None
k8s_client_initialized = False

def initialize_k8s_client():
    """Initializes Kubernetes API clients (CoreV1Api, VersionApi)."""
    # --- MODIFIED: Initialize k8s_version_api ---
    global k8s_core_v1, k8s_apps_v1, k8s_version_api, k8s_client_initialized
    if k8s_client_initialized:
        logging.debug("[K8s Monitor] Client already initialized.")
        return True
    print("[DEBUG_PRINT] Attempting to initialize K8s client...", flush=True) # Keep for initial debug
    ...
    try:
        ...
        k8s_version_api = client.VersionApi() # Initialize Version API
        # k8s_apps_v1 = client.AppsV1Api() # Initialize if needed later
...
# --- NEW FUNCTION: Get basic cluster summary ---
def get_cluster_summary():
    """Retrieves basic cluster information (version, node count)."""
    ...
    try:
        if k8s_core_v1:
            nodes = k8s_core_v1.list_node(limit=1) # Limit=1 is enough to get total count
    ...
    try:
        ...
        if events and events.items:
                ...
                sorted_events = sorted(
                    events.items,
                    # Handle cases where last_timestamp might be None
                ...
                for event in sorted_events:
                    ...
                    if len(recent_events) >= 15:
                        break
                    # Stop if events are older than the threshold
                    if event_time and event_time < since_time:
                        break
        return recent_events
    except ApiException as e:
        # Ignore forbidden errors (RBAC), log others
        if e.status != 403:
...
def format_k8s_context(pod_info, node_info, pod_events):
    ...
    if pod_info:
        ...
        # Add Problematic Pod Conditions
        if pod_info.get('conditions'):
                problematic_conditions = [
                    ...
                    # List conditions that indicate a problem
    ...
    if node_info:
        ...
        # Add Problematic Node Conditions
        if node_info.get('conditions'):
                problematic_conditions = [
                    f"{ctype}({cinfo.get('status','N/A')}-{cinfo.get('reason','N/A')})"
                    for ctype, cinfo in node_info['conditions'].items()
                    # List conditions that indicate a problem (e.g., NotReady, or True status for issues like MemoryPressure)
                    if (ctype == 'Ready' and cinfo.get('status') != 'True') or \
...
def scan_kubernetes_for_issues(namespaces_to_scan, restart_threshold, loki_detail_log_range_minutes=30):
    ...
    # Calculate time threshold for considering recent terminations
    ...
    for ns in namespaces_to_scan:
        try:
            ...
            for pod in pods.items:
                ...
                # Check Container Statuses if no pod-level issue found yet
                if not issue_found and pod.status.container_statuses:
                    for cs in pod.status.container_statuses:
                        ...
                        if cs.restart_count >= restart_threshold:
                            ...
                            break # Found an issue, move to next pod
                        # Check Problematic Waiting States
                        ...
                        # Check Problematic Terminated States (recent or non-Always restart policy)
                        if cs.state and cs.state.terminated and cs.state.terminated.reason in [
                            ...
                            # Check if termination time is available and recent
                            if finished_at_str:
                                try:
                                    # finished_at is already timezone-aware
                            ...
                            # Flag as issue if termination is recent OR if the pod won't restart anyway
                ...
                # If an issue was found for this pod, add it to the results
                if issue_found:
                    ...
                    if pod_key not in problematic_pods:
                        problematic_pods[pod_key] = {
                            ...
                            "reason": f"K8s: {reason}" # Prefix reason with K8s:
                        }
        except ApiException as e:
