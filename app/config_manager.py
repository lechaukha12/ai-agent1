...
DEFAULT_K8S_CLUSTER_NAME = "unknown-cluster" # Đổi tên thành DEFAULT_ENVIRONMENT_NAME cho nhất quán
DEFAULT_ENVIRONMENT_NAME = DEFAULT_K8S_CLUSTER_NAME # Thêm dòng này

AGENT_ID = os.environ.get("AGENT_ID")
if not AGENT_ID:
    logging.warning("AGENT_ID environment variable not set. Using K8S_CLUSTER_NAME/ENVIRONMENT_NAME as fallback.")
    # Ưu tiên ENVIRONMENT_NAME nếu có, nếu không thì dùng K8S_CLUSTER_NAME
    AGENT_ID = os.environ.get("ENVIRONMENT_NAME", os.environ.get("K8S_CLUSTER_NAME", DEFAULT_ENVIRONMENT_NAME))
...
# Hàm async để fetch config từ API với retry (Giữ nguyên)
...
def _load_config_from_env_defaults():
    ...
    # --- Đọc tên môi trường từ ENV ---
    # Ưu tiên biến ENVIRONMENT_NAME nếu có, nếu không thì dùng K8S_CLUSTER_NAME
    config['environment_name'] = os.environ.get("ENVIRONMENT_NAME", os.environ.get("K8S_CLUSTER_NAME", DEFAULT_ENVIRONMENT_NAME))
    # ---------------------------------
    ...
    # Lưu lại giá trị env này để dùng làm fallback nếu API không trả về namespaces
    config['monitored_namespaces_env'] = [ns.strip() for ns in monitored_ns_str_env.split(',') if ns.strip()]
    # Đặt giá trị ban đầu cho monitored_namespaces từ env
    config['monitored_namespaces'] = config['monitored_namespaces_env']
...
async def get_config(session, force_refresh=False):
    ...
    async with _config_refresh_lock:
        ...
        merged_config = env_defaults.copy() # Bắt đầu với default từ env
        ...
        if api_config and isinstance(api_config, dict): # Kiểm tra api_config hợp lệ
            ...
            except (ValueError, TypeError, KeyError): pass # Bỏ qua nếu không có hoặc không hợp lệ

            # Ví dụ cho restart_count_threshold (chỉ áp dụng nếu agent là K8s)
            # Cần biết loại agent để áp dụng đúng config. Tạm thời áp dụng nếu có trong API.
            try:
            ...
            # Ưu tiên monitored_namespaces từ API nếu nó là list và không rỗng
            ...
            # Nếu API không trả về list hợp lệ, monitored_namespaces sẽ giữ nguyên giá trị từ env_defaults

            # --- Xử lý các config khác từ API nếu có ---
            # Ví dụ:
            # if 'some_other_config' in api_config:
            #     merged_config['some_other_config'] = api_config['some_other_config']
            # -----------------------------------------
            ...
            # Đảm bảo monitored_namespaces vẫn giữ giá trị từ env nếu API lỗi
            merged_config['monitored_namespaces'] = merged_config.get('monitored_namespaces_env', [])
            if not merged_config['monitored_namespaces']:
                 # Fallback cuối cùng nếu env cũng không có
                 merged_config['monitored_namespaces'] = [ns.strip() for ns in DEFAULT_MONITORED_NAMESPACES_STR.split(',') if ns.strip()]


        # Loại bỏ key tạm thời
        merged_config.pop('monitored_namespaces_env', None)

        # --- FIX: Đảm bảo key 'environment_name' tồn tại ---
        # Giá trị này đã được đọc từ ENV trong _load_config_from_env_defaults
        # Không cần đổi tên ở đây nữa.
        # if 'cluster_name' in merged_config:
        #    merged_config['environment_name'] = merged_config.pop('cluster_name')
        # elif 'environment_name' not in merged_config: # Đảm bảo nó tồn tại
        #    merged_config['environment_name'] = DEFAULT_ENVIRONMENT_NAME
        # -------------------------------------------------
        ...
        logging.info(f"[Config Manager] Current effective config: EnvName={_current_agent_config.get('environment_name')}, ScanInterval={_current_agent_config.get('scan_interval_seconds')}, RestartThreshold={_current_agent_config.get('restart_count_threshold')}, LokiLevel={_current_agent_config.get('loki_scan_min_level')}, Namespaces={_current_agent_config.get('monitored_namespaces')}")
...
def get_monitored_namespaces():
    ...
    # --- FIX: Trả về bản copy ---
    return _current_agent_config.get('monitored_namespaces', []).copy()
    # ---------------------------

def get_environment_name(): # Đổi tên hàm
    return _current_agent_config.get('environment_name', DEFAULT_ENVIRONMENT_NAME)
